...
// Wheel trim, in servo units either side of the nominal stop. Applied by
// tasks_joysticks(); persisted in NVS beside the Level choice.
extern int8_t remotexy_get_trim_l(void);
extern int8_t remotexy_get_trim_r(void);
// Deferred flash write plus the four-way echo between a wheel's controls.
extern void   remotexy_trim_tick(void);
